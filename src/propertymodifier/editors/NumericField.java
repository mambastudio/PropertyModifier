...
public class NumericField extends TextField
{
    ...
    private void initFloatFormatter()
    {
        //unary filter 
        UnaryOperator<TextFormatter.Change> filter = change -> {
            String text = change.getControlNewText();
        ...
        //convert string to float when not in use
        StringConverter<Float> convert = new StringConverter<Float>() {
            @Override
        ...
        TextFormatter textFormatter = new TextFormatter<>(convert, 0.0f, filter);                                    
        setTextFormatter(textFormatter);
        //setMaxWidth(Double.MAX_VALUE);
        setOnKeyPressed(event -> 
        {
            if(event.getCode() == KeyCode.ENTER){
                FloatProperty prop = (FloatProperty)value;
                String text = getText();
                if(text != null && !text.isEmpty())
                {
                    prop.setValue(Float.valueOf(getText()));
                    this.positionCaret(text.lastIndexOf(text));
                    Platform.runLater(()->this.selectAll());
                }
            }
